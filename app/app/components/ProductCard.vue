...
const props = defineProps<{
  ...
  eager?: boolean;
  /**
   * Карточка вставлена скриптом, а не пришла в документе.
   *
   * Только такие проявляются: про них известно, что снимка ещё нет. Серверные
   * оставляем как есть — их снимок часто приходит раньше, чем выполнится код
   * фреймворка, и гасить уже нарисованное значило бы мигать.
   */
  fresh?: boolean;
  product?: Product;
}>();
...
/**
 * Проявление — только у догруженных карточек, и решается прямо здесь.
 *
 * Раньше этим занимался наблюдатель, вписанный прямо в head документа. Он был
 * нужен, потому что снимки серверных карточек часто приходят РАНЬШЕ, чем
 * выполнится код фреймворка, и обработчик Vue их не заставал — терялось семь
 * карточек из тридцати.
 *
 * Теперь серверные карточки не проявляются вовсе: их снимок просто закрывает
 * размытое превью собой, и ловить момент незачем. Наблюдатель стал не нужен,
 * а документ избавился от инлайнового блока скриптов — осталась одна ссылка
 * на точку входа.
 *
 * Догруженные карточки вставляем мы сами и точно знаем, что снимка ещё нет:
 * здесь обработчик успевает всегда, и мигания быть не может.
 */
const revealed = ref(false);

/** Проявляется ли эта карточка. Серверные — нет. */
const fades = computed(() => Boolean(props.fresh));
...
<template>
  <div class="pcard" @mouseenter="prefetchProduct" @focusin="prefetchProduct">
    <a class="pcard-link" :href="product?.href ?? '#'">
      <!--
        Проявляются только догруженные карточки. У серверных снимок часто
        приходит раньше кода фреймворка, и ловить этот момент бессмысленно —
        он просто закрывает превью собой.
      -->
      <div
        class="pcard-media"
        :class="[
          hasPh ? ['has-ph', placeholder ? '' : `ph-${phKey}`] : [],
          fades ? (revealed ? ['is-pending', 'is-loaded'] : 'is-pending') : [],
        ]"
        :style="phStyle"
      >
        <img
