...
/**
 * Что именно мерим: первый визит или повторный.
 *
 * Раньше выбора не было — номер прогона всегда уходил в адрес кадра и делал его
 * уникальным, поэтому кеш браузера не мог сработать в принципе, и «С начала»
 * означало «первый визит», о чём нигде не было сказано.
 *
 * `cold` — адрес уникален, всё качается заново. `warm` — адрес постоянный,
 * и документ берётся из кеша браузера либо подтверждается через 304.
 */
const cacheMode = ref<'cold' | 'warm'>('cold');

const query = computed(() =>
  cacheMode.value === 'cold'
    ? `ph=${ph.value}&cold=${runId.value}`
    : `ph=${ph.value}`,
);

/** Подпись режима — обязана быть рядом с цифрами, иначе их не с чем сопоставить. */
const modeLabel = computed(() =>
  cacheMode.value === 'cold'
    ? 'первый визит: кеш пуст'
    : 'повторный визит: документ и скрипты из кеша',
);
...
<template>
  <div class="cmp">
      ...
      <div class="cmp-row">
        ...
        <label>
          ...
          <select v-model="cacheMode">
            <option value="cold">первый визит</option>
            <option value="warm">повторный</option>
          </select>
        </label>
    ...
    <div class="cmp-warn">
      <b>Переключатель «прогон» решает, что именно вы меряете.</b>
      На ПЕРВОМ визите серверный рендер выигрывает: карточки приезжают готовыми
      в документе, тогда как клиентской стратегии надо сначала скачать и выполнить
      79 КБ скриптов и только потом сходить за данными. На ПОВТОРНОМ документ
      берётся из кеша браузера либо подтверждается ответом 304 без тела —
      сеть в этом случае почти не участвует.
      <br><br>
      Два кадра грузятся ОДНОВРЕМЕННО и делят канал, поэтому глазами сравнивать
      их некорректно — смотрите на числа в шапках панелей.
    </div>
    ...
    <div class="metrics">
      <section class="mblock">
        <div class="mhead">
          ...
          <span class="dim">
            Миллисекунды от начала загрузки. Замерено в самих кадрах.
            Режим: <b>{{ modeLabel }}</b>.
          </span>
        </div>
