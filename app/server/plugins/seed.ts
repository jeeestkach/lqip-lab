...
/**
 * Размеры копий: карточка каталога и страница товара.
 *
 * Раньше их было три ради `srcset`. Теперь карточка берёт единственную копию
 * ровно под свой слот на ретине (`CARD_WIDTH`), а вторая, крупная, остаётся
 * странице товара. Третья ширина обслуживала только выбор внутри `srcset`
 * и без него превратилась в файлы, которые никто не запрашивает.
 */
const SIZES = [CARD_WIDTH, 900];
...
export default defineNitroPlugin(() => {
  void (async () => {
    try {
       ...
       *
       * Сбрасываем ЯВНО, а не по сроку годности: срок пришлось бы угадывать,
